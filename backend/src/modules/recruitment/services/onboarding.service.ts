...
export const OnboardingService = {
  ...
  async start(context: RecruitmentActorContext, candidateLeadId: string, offerLetterId: string, startDate: Date) {
    await CandidateService.getById(context.companyId, candidateLeadId);

    const existing = await OnboardingRepository.findOne({ candidateLeadId }, { companyId: context.companyId });
    if (existing) {
      return existing;
    }
    ...
    await CandidatePipelineService.transition(context, candidateLeadId, PIPELINE_STAGE.ONBOARDING);
  ...
  async saveDraft(context: RecruitmentActorContext, candidateLeadId: string, section: string, data: Record<string, unknown>) {
    ...
    const updated = await OnboardingRepository.update(
      onboarding.id,
      { formData, currentSection: section, status: ONBOARDING_STATUS.IN_PROGRESS, updatedBy: context.userId },
      { companyId: context.companyId },
    );
  ...
  async completeSection(context: RecruitmentActorContext, candidateLeadId: string, section: string, data: Record<string, unknown>) {
    const onboarding = await this.getByCandidate(context.companyId, candidateLeadId);
    ...
    const checklistItems = onboarding.checklistItems.map((item) =>
      item.title === section ? { ...item, isCompleted: true, completedAt: new Date(), completedBy: context.userId } : item,
    );
  ...
  async issuePortalLink(context: RecruitmentActorContext, candidateLeadId: string) {
    const onboarding = await OnboardingRepository.findOne({ candidateLeadId }, { companyId: context.companyId });
    if (!onboarding) {
      throw new NotFoundError('Onboarding not found — start onboarding first', ERROR_CODES.NOT_FOUND);
    }
