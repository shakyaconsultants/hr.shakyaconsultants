...
import { apiKeyAuthMiddleware } from '@modules/integration/middleware/api-key-auth.middleware.js';
...
/** Face recognition / external kiosk — API key auth only */
attendanceRoutes.post('/external-punch', apiKeyAuthMiddleware, externalPunch);
...
attendanceRoutes.get(
  '/shift-assignments',
  authorize(ATTENDANCE_PERMISSIONS.SHIFT_READ),
  listShiftAssignments,
);
attendanceRoutes.post(
  '/shift-assignments',
  authorize(ATTENDANCE_PERMISSIONS.SHIFT_CREATE),
  createShiftAssignment,
);
attendanceRoutes.patch(
  '/shift-assignments/:id',
  authorize(ATTENDANCE_PERMISSIONS.SHIFT_UPDATE),
  updateShiftAssignment,
);
attendanceRoutes.delete(
  '/shift-assignments/:id',
  authorize(ATTENDANCE_PERMISSIONS.SHIFT_DELETE),
  deleteShiftAssignment,
);
...
attendanceRoutes.post(
  '/corrections/:id/submit',
  authorize(ATTENDANCE_PERMISSIONS.CREATE),
  submitCorrection,
);
...
attendanceRoutes.post(
  '/processing/monthly',
  authorize(ATTENDANCE_PERMISSIONS.UPDATE),
  processMonthly,
);
