...
import { ATTENDANCE_AUDIT_WHERE } from '@modules/attendance/constants/attendance.constants.js';

export interface AttendanceAuditInput {
  companyId: string;
  userId: string;
  entityType: string;
  entityId: string;
  action: 'create' | 'update' | 'delete' | 'approve' | 'reject' | 'punch' | 'process';
...
const ATTENDANCE_AUDIT_ACTION_MAP: Record<AttendanceAuditInput['action'], string> = {
...
export const AttendanceAuditService = {
  ...
  async log(input: AttendanceAuditInput): Promise<void> {
    const correlationId = getCorrelationId() ?? 'system';
    const changes = { before: input.before ?? null, after: input.after ?? null, correlationId };

    AuditLogService.log({
      who: input.userId,
      where: ATTENDANCE_AUDIT_WHERE,
      action: ATTENDANCE_AUDIT_ACTION_MAP[input.action],
