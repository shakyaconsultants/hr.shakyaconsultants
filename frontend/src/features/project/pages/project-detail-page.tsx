...
export function ProjectDetailPage() {
  ...
  const isProjectManager = Boolean(
    project && employeeId && project.projectManagerId === employeeId,
  );
            ...
            {dashboard && (
              <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  ...
                  <dd className="text-xl font-semibold">{dashboard.totalTasks}</dd>
                </div>
                <div>
                  <dt className="text-sm text-muted-foreground">Blocked</dt>
                  <dd className="text-xl font-semibold">{dashboard.blockedTasks}</dd>
                </div>
                <div>
                  <dt className="text-sm text-muted-foreground">Overdue</dt>
                  <dd className="text-xl font-semibold">{dashboard.overdueTasks}</dd>
                </div>
                <div>
                  <dt className="text-sm text-muted-foreground">Pending Verifications</dt>
                  <dd className="text-xl font-semibold">{dashboard.pendingVerifications ?? 0}</dd>
                </div>
              </dl>
            )}
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              {project.requirements && (
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-1 whitespace-pre-wrap">{project.requirements}</dd>
                </div>
              )}
              {project.uiDocs && (
                <div className="sm:col-span-2">
                  <dt className="text-sm text-muted-foreground">UI / Design Docs</dt>
                  <dd className="mt-1 whitespace-pre-wrap">{project.uiDocs}</dd>
                </div>
              )}
              {project.scalabilityNotes && (
                <div className="sm:col-span-2">
                  <dt className="text-sm text-muted-foreground">Scalability & Tech Stack</dt>
                  <dd className="mt-1 whitespace-pre-wrap">{project.scalabilityNotes}</dd>
                </div>
              )}
              {project.startDate && (
                <div>
              ...
              {project.endDate && (
                <div>
                  ...
                  <dd>{new Date(project.endDate).toLocaleDateString()}</dd>
                </div>
              )}
              {project.clientName && (
                <div>
        ...
        {activeTab === 'Kanban' && kanban && (
          <div className="flex gap-4 overflow-x-auto pb-4">
            {Object.entries(kanban.columns ?? {}).map(([status, cards]) => (
              <div
                key={status}
                className="min-w-[260px] flex-shrink-0 rounded-lg border bg-muted/30"
              >
            ...
            <div>
              ...
              <dd className="mt-1">
                {knowledgeBase?.repositoryUrl ? (
                  <a
                    href={knowledgeBase.repositoryUrl}
                    target="_blank"
                    rel="noreferrer"
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  ...
                  <Button
                    ...
                    disabled={!knowledgeBase?.envVariables}
                  >
                    Export .env
                  </Button>
                </div>
