...
function mergeDraft(
  ...
  const merged = {
    ...
    basicInfo: {
      ...local.basicInfo,
      ...(remote.basicInfo as ProjectWizardDraft['basicInfo'] | undefined),
    },
    requirements: {
      ...local.requirements,
      ...(remote.requirements as ProjectWizardDraft['requirements'] | undefined),
    },
    tech: { ...local.tech, ...(remote.tech as ProjectWizardDraft['tech'] | undefined) },
    deployment: {
      ...local.deployment,
      ...(remote.deployment as ProjectWizardDraft['deployment'] | undefined),
    },
...
function buildFinalizePayload(draft: ProjectWizardDraft) {
  const tags = draft.tech.tags
    .split(',')
    .map((t) => t.trim())
    .filter(Boolean);
  const documentUrls = draft.requirements.documentUrls
    .split('\n')
    .map((t) => t.trim())
    .filter(Boolean);
  const requirements = [draft.requirements.goals, draft.requirements.functionality]
    .filter(Boolean)
    .join('\n\n');
    ...
    basicInfo: {
      ...
      clientName:
        draft.basicInfo.projectKind === 'external'
          ? draft.basicInfo.clientName || undefined
          : undefined,
...
export function ProjectCreateWizardPage() {
  ...
  const [draft, setDraft] = useState<ProjectWizardDraft>(
    () => loadLocalWizardDraft() ?? EMPTY_PROJECT_WIZARD_DRAFT,
  );
  ...
  useEffect(() => {
    if (hydratedFromRemote.current || !remoteDraft?.payload) return;
  ...
  const employeeName = useMemo(() => {
    ...
    const map = new Map(
      merged.map((e) => [e.id, `${e.firstName ?? ''} ${e.lastName ?? ''}`.trim()]),
    );
    return (id: string) => map.get(id) ?? id;
  }, [employeeOptions, projectManagers]);

  async function persistDraft(nextIndex?: number, snapshot?: ProjectWizardDraft) {
    if (saveDraftMutation.isPending) return;
    const currentDraft = snapshot ?? draftRef.current;
    const index = nextIndex ?? currentDraft.currentStepIndex;
    const currentStep = PROJECT_WIZARD_STEPS[index]?.id ?? step.id;
    setSaving(true);
    setError(null);
    const saved = await runFormMutation({
      ...
      mutation: () =>
        saveDraftMutation.mutateAsync({
          currentStep,
          payload: currentDraft as unknown as Record<string, unknown>,
        }),
    });
    setSaving(false);
    if (!saved) return;
  ...
  function nextStep() {
    setDraft((prev) => {
      const nextIndex = Math.min(prev.currentStepIndex + 1, PROJECT_WIZARD_STEPS.length - 1);
      const nextDraft = { ...prev, currentStepIndex: nextIndex };
      void persistDraft(nextIndex, nextDraft);
      return nextDraft;
    });
  }
  ...
  async function handleFinalize() {
    if (!draft.basicInfo.name || !draft.basicInfo.projectManagerId) {
      setError('Name and project manager are required.');
      return;
    }
  ...
  function addTeamMember() {
    setDraft((prev) => ({
      ...
      teamMembers: [
        ...prev.teamMembers,
        { employeeId: '', role: 'developer', allocationPercent: 100 },
      ],
    }));
  }
  ...
  function exportEnvFile() {
    ...
    a.href = url;
    a.download = `${draft.basicInfo.name.replace(/[^a-zA-Z0-9]/g, '-').slice(0, 24) || 'project'}-env.txt`;
    a.click();
        ...
        <section className="rounded-lg border bg-card p-6">
          ...
          {step.id === 'basic' && (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <p className="sm:col-span-2 text-sm text-muted-foreground">
                A unique project code will be generated automatically when the project is created.
              </p>
              <Field label="Project Type *">
                <select
                  className="h-10 w-full rounded-md border bg-background px-3 text-sm"
                  value={draft.basicInfo.projectKind}
                  onChange={(e) => updateBasic('projectKind', e.target.value)}
                >
              ...
              {draft.basicInfo.projectKind === 'external' && (
                <Field label="Client Name">
                  <Input
                    value={draft.basicInfo.clientName}
                    onChange={(e) => updateBasic('clientName', e.target.value)}
                  />
                </Field>
              )}
              <Field label="Start Date (optional)">
                <DatePicker
                  value={draft.basicInfo.startDate}
                  onChange={(value) => updateBasic('startDate', value)}
                />
              </Field>
              <Field label="End Date (optional)">
                <DatePicker
                  value={draft.basicInfo.endDate}
                  onChange={(value) => updateBasic('endDate', value)}
                  min={draft.basicInfo.startDate || undefined}
                />
              </Field>
            ...
            <div className="space-y-4">
              <Field label="Project Goals">
                <textarea
                  ...
                  value={draft.requirements.goals}
                  onChange={(e) => updateRequirements('goals', e.target.value)}
                  placeholder="What should this project achieve?"
                />
              </Field>
              <Field label="Functionality & Scope">
                <textarea
                  className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
                  value={draft.requirements.functionality}
                  onChange={(e) => updateRequirements('functionality', e.target.value)}
                  placeholder="Core features and user flows"
                />
              </Field>
              <Field label="UI / Design Docs">
                <textarea
                  className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
                  value={draft.requirements.uiDocs}
                  onChange={(e) => updateRequirements('uiDocs', e.target.value)}
                  placeholder="Wireframes, Figma links, design notes"
                />
              </Field>
              <Field label="Reference Document URLs (one per line)">
                <textarea
                  className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
                  value={draft.requirements.documentUrls}
                  onChange={(e) => updateRequirements('documentUrls', e.target.value)}
                />
              </Field>
            </div>
          )}
            ...
            <div className="space-y-4">
              <Field label="Scalability Notes">
                <textarea
                  ...
                  value={draft.tech.scalabilityNotes}
                  onChange={(e) => updateTech('scalabilityNotes', e.target.value)}
                  placeholder="Architecture, scaling strategy, infra choices..."
                />
              </Field>
              <Field label="Tags (comma-separated)">
                <Input
                  value={draft.tech.tags}
                  onChange={(e) => updateTech('tags', e.target.value)}
                  placeholder="backend, mobile, api"
                />
              </Field>
            </div>
          )}
          ...
          {step.id === 'deployment' && (
            <div className="space-y-4">
              <Field label="GitHub / Repository URL">
                <Input
                  value={draft.deployment.repositoryUrl}
                  onChange={(e) => updateDeployment('repositoryUrl', e.target.value)}
                  placeholder="https://github.com/org/repo"
                />
              </Field>
              <Field label="Production Deploy URL">
                <Input
                  value={draft.deployment.productionDeployUrl}
                  onChange={(e) => updateDeployment('productionDeployUrl', e.target.value)}
                  placeholder="https://app.example.com"
                />
              </Field>
              <Field label="Development Deploy URL">
                <Input
                  value={draft.deployment.developmentDeployUrl}
                  onChange={(e) => updateDeployment('developmentDeployUrl', e.target.value)}
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={!draft.deployment.envVariables}
                    >
                      Export .env file
                    </Button>
                  </div>
                  <textarea
                    className={cn(
                      'min-h-32 w-full rounded-md border bg-background px-3 py-2 font-mono text-sm',
                      !showEnv && 'text-security-disc',
                    )}
                    style={
                      !showEnv ? ({ WebkitTextSecurity: 'disc' } as React.CSSProperties) : undefined
                    }
              ...
              <Field label="Deployment Notes">
                <textarea
                  className="min-h-24 w-full rounded-md border bg-background px-3 py-2 text-sm"
                  value={draft.deployment.deploymentGuide}
                  onChange={(e) => updateDeployment('deploymentGuide', e.target.value)}
                />
              </Field>
            </div>
          )}
                ...
                <div key={index} className="grid gap-3 rounded border p-4 sm:grid-cols-3">
                  <Field label="Employee">
                    ...
                    >
                      ...
                      {employeeOptions
                        .filter((e) => e.id !== draft.basicInfo.projectManagerId)
                        .map((e) => (
                  ...
                  <Field label="Allocation %">
                    <Input
                      ...
                      min={0}
                      max={100}
                      value={member.allocationPercent ?? 100}
                      onChange={(e) =>
                        updateTeamMember(index, 'allocationPercent', Number(e.target.value))
                      }
                    />
                  </Field>
                </div>
          ...
          {step.id === 'review' && (
            <div className="space-y-3 text-sm">
              <ReviewRow label="Project" value={draft.basicInfo.name} />
              <ReviewRow label="Project code" value="Generated automatically on create" />
              ...
              <ReviewRow
                label="Team Members"
                value={String(draft.teamMembers.filter((m) => m.employeeId).length)}
              />
              <ReviewRow label="Tags" value={draft.tech.tags || '—'} />
              <ReviewRow
                label="Env Variables"
                value={draft.deployment.envVariables ? 'Configured (encrypted)' : 'None'}
              />
            </div>
          )}
