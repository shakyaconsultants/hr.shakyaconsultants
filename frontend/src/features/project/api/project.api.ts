...
import { asRecord, unwrapApiPaginated } from '@/shared/utils/api-normalize.util';
...
export async function fetchManagerDashboard(): Promise<ManagerDashboard> {
  const { data } = await apiClient.get<ApiSuccessResponse<ManagerDashboard>>(
    `${PROJECT_PREFIX}/dashboard/manager`,
  );
  return data.data;
}

export async function fetchEnterpriseDashboard(): Promise<EnterpriseDashboard> {
  const { data } = await apiClient.get<ApiSuccessResponse<EnterpriseDashboard>>(
    `${PROJECT_PREFIX}/dashboard/enterprise`,
  );
  return data.data;
}
...
export async function fetchDeveloperDashboard(): Promise<Record<string, unknown>> {
  const { data } = await apiClient.get<ApiSuccessResponse<Record<string, unknown>>>(
    `${PROJECT_PREFIX}/dashboard/developer`,
  );
  return data.data;
}
...
export async function fetchProjects(
  ...
  const { data } = await apiClient.get<ApiSuccessResponseWithPagination<ProjectRecord>>(
    PROJECT_PREFIX,
    { params },
  );
  return unwrapApiPaginated<ProjectRecord>(data, params.pageSize ?? 20);
}

export async function fetchProject(id: string): Promise<ProjectRecord> {
  const { data } = await apiClient.get<ApiSuccessResponse<ProjectRecord>>(
    `${PROJECT_PREFIX}/${id}`,
  );
  return data.data;
}
...
export async function updateProject(
  ...
  const { data } = await apiClient.patch<ApiSuccessResponse<ProjectRecord>>(
    `${PROJECT_PREFIX}/${id}`,
    payload,
  );
  return data.data;
...
export async function archiveProject(id: string): Promise<ProjectRecord> {
  const { data } = await apiClient.post<ApiSuccessResponse<ProjectRecord>>(
    `${PROJECT_PREFIX}/${id}/archive`,
  );
  return data.data;
}
...
export async function fetchProjectModules(projectId: string): Promise<Record<string, unknown>[]> {
  const { data } = await apiClient.get<ApiSuccessResponse<Record<string, unknown>[]>>(
    `${PROJECT_PREFIX}/${projectId}/modules`,
  );
  return data.data;
}
...
export async function fetchProjectMilestones(
  ...
  const { data } = await apiClient.get<ApiSuccessResponse<Record<string, unknown>[]>>(
    `${PROJECT_PREFIX}/${projectId}/milestones`,
  );
  return data.data;
}
...
export async function updateProjectSprint(
  ...
  const { data } = await apiClient.patch<ApiSuccessResponse<Record<string, unknown>>>(
    `${PROJECT_PREFIX}/sprints/${sprintId}`,
    payload,
  );
  return data.data;
}
...
export async function fetchProjectKnowledgeBase(
  projectId: string,
): Promise<ProjectKnowledgeBase | null> {
  const { data } = await apiClient.get<ApiSuccessResponse<ProjectKnowledgeBase | null>>(
...
export async function fetchProjectSprints(projectId: string): Promise<Record<string, unknown>[]> {
  const { data } = await apiClient.get<ApiSuccessResponse<Record<string, unknown>[]>>(
    `${PROJECT_PREFIX}/${projectId}/sprints`,
  );
  return data.data;
}
...
export async function fetchTask(id: string): Promise<TaskRecord> {
  const { data } = await apiClient.get<ApiSuccessResponse<TaskRecord>>(
    `${PROJECT_PREFIX}/tasks/${id}`,
  );
  return data.data;
}
...
export async function fetchTaskVerifications(taskId: string): Promise<TaskVerificationRecord[]> {
  const { data } = await apiClient.get<ApiSuccessResponse<TaskVerificationRecord[]>>(
    `${PROJECT_PREFIX}/tasks/${taskId}/verifications`,
  );
  return data.data;
}
...
export async function submitTaskVerification(
  ...
  const { data } = await apiClient.post<ApiSuccessResponse<TaskVerificationRecord>>(
    `${PROJECT_PREFIX}/tasks/${taskId}/verify`,
    { verifierId },
  );
  return data.data;
}

export async function approveTaskVerification(
  verificationId: string,
  comment?: string,
): Promise<TaskVerificationRecord> {
  const { data } = await apiClient.post<ApiSuccessResponse<TaskVerificationRecord>>(
    `${PROJECT_PREFIX}/verifications/${verificationId}/approve`,
    { comment },
  );
  return data.data;
}

export async function rejectTaskVerification(
  verificationId: string,
  comment: string,
  revisionNotes?: string,
): Promise<TaskVerificationRecord> {
  const { data } = await apiClient.post<ApiSuccessResponse<TaskVerificationRecord>>(
    `${PROJECT_PREFIX}/verifications/${verificationId}/reject`,
...
export async function fetchProjectDashboard(projectId: string): Promise<ManagerDashboard> {
  const { data } = await apiClient.get<ApiSuccessResponse<ManagerDashboard>>(
    `${PROJECT_PREFIX}/dashboard/${projectId}`,
  );
  return data.data;
}

export async function fetchWizardDraft(): Promise<ProjectWizardDraftResponse | null> {
  const { data } = await apiClient.get<ApiSuccessResponse<ProjectWizardDraftResponse | null>>(
    `${PROJECT_PREFIX}/wizard/draft`,
  );
  return data.data;
}
...
export async function saveWizardDraft(payload: {
  currentStep: string;
  payload: Record<string, unknown>;
}): Promise<ProjectWizardDraftResponse> {
  const { data } = await apiClient.put<ApiSuccessResponse<ProjectWizardDraftResponse>>(
    `${PROJECT_PREFIX}/wizard/draft`,
    payload,
  );
  return data.data;
...
export async function finalizeProjectWizard(
  payload: Record<string, unknown>,
): Promise<ProjectRecord> {
  const { data } = await apiClient.post<ApiSuccessResponse<ProjectRecord>>(
    `${PROJECT_PREFIX}/wizard/finalize`,
    payload,
  );
  return data.data;
}
...
export async function fetchMemberHistory(projectId: string): Promise<Record<string, unknown>[]> {
  const { data } = await apiClient.get<ApiSuccessResponse<Record<string, unknown>[]>>(
    `${PROJECT_PREFIX}/${projectId}/members/history`,
  );
  return data.data;
}
