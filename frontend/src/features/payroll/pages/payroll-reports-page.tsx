...
import type { ReportParams } from '@/features/payroll/api/payroll.api';
import { useExportPayrollReport, usePayrollReport } from '@/features/payroll/hooks/use-payroll';
...
export function PayrollReportsPage() {
  ...
  const { data: report, isLoading, refetch } = usePayrollReport(params);
  ...
  const handleExport = async () => {
    ...
    link.href = url;
    link.download = `payroll-report-${params.startDate}-${params.endDate}.csv`;
    link.click();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Generate payroll reports and export to CSV.</p>
      </div>
