...
import { formatProjectRole } from '@/features/project/utils/project-display.util';
...
export function WorkspaceProjectsPage() {
  ...
  return (
    <div className="space-y-6">
      <WorkspacePageHeader
        title="My Projects"
        description="Projects you are assigned to — view details and manage your tasks."
      />
      {(data?.items.length ?? 0) === 0 ? (
        <EmptyState
          title="No projects assigned"
          description="When you are added to a project team, it will appear here."
        />
      ) : (
            ...
            return (
              <article key={projectId} className="rounded-lg border bg-card p-5">
                <div className="mb-3 flex items-start justify-between">
                  <div>
                    <Link
                      to={ROUTES.workspaceProjectDetail(projectId)}
                ...
                <p className="text-xs text-muted-foreground">
                  {item.myTaskCount ?? 0} tasks assigned to you · {item.completedTasks}/
                  {item.totalTasks} project tasks done
                </p>
                <div className="mt-3 flex flex-wrap gap-3">
                  ...
                  <Link
                    to={ROUTES.workspaceProjectTasks(projectId)}
                  ...
                  <div className="mt-3 border-t pt-3">
                    ...
                    <ul className="text-xs text-muted-foreground">
                      {item.upcomingDeadlines.slice(0, 3).map((d) => (
                        <li key={String(d.id)}>
                          {String(d.title)} —{' '}
                          {d.dueDate ? new Date(String(d.dueDate)).toLocaleDateString() : ''}
                        </li>
                      ))}
                    </ul>
                  </div>
