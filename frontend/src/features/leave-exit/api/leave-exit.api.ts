...
async function unwrapPaginated<T>(response: {
  data:
    ...
    | (ApiSuccessResponse<PaginatedResult<T>> & { pagination?: PaginatedResult<T>['pagination'] });
  ...
  const pagination = (response.data as { pagination?: PaginatedResult<T>['pagination'] })
    .pagination ?? { page: 1, pageSize: items.length || 20, total: items.length, totalPages: 1 };
...
export async function fetchLeavePolicies(): Promise<LeavePolicy[]> {
  const response = await apiClient.get<ApiSuccessResponse<LeavePolicy[]>>(
    `${LEAVE_EXIT_PREFIX}/policies`,
  );
...
export async function fetchLeaveBalances(
  ...
  const response = await apiClient.get<ApiSuccessResponse<LeaveBalance[]>>(
    `${LEAVE_EXIT_PREFIX}/balances`,
    {
...
export async function adjustLeaveBalance(payload: {
  ...
  const response = await apiClient.post<ApiSuccessResponse<LeaveBalance>>(
    `${LEAVE_EXIT_PREFIX}/balances/adjust`,
    payload,
  );
  return unwrap(response);
}
...
export async function seedLeaveDefaults(): Promise<{ seeded: boolean }> {
  const response = await apiClient.post<ApiSuccessResponse<{ seeded: boolean }>>(
    `${LEAVE_EXIT_PREFIX}/seed-defaults`,
  );
  return unwrap(response);
}
...
export async function fetchLeaveRequests(
  ...
  >(`${LEAVE_EXIT_PREFIX}/leave-requests`, {
...
export async function withdrawLeave(id: string): Promise<LeaveRequest> {
  const response = await apiClient.post<ApiSuccessResponse<LeaveRequest>>(
    `${LEAVE_EXIT_PREFIX}/leave-requests/${id}/withdraw`,
  );
  return unwrap(response);
}
...
export async function fetchLeaveCalendar(
  ...
  const response = await apiClient.get<ApiSuccessResponse<LeaveRequest[]>>(
    `${LEAVE_EXIT_PREFIX}/leave-requests/calendar`,
    {
...
function normalizeCalendarEvents(
  data: CalendarEvent[] | CompanyCalendarResponse | undefined,
): CalendarEvent[] {
...
export async function fetchCompanyCalendar(
  ...
  const response = await apiClient.get<
    ApiSuccessResponse<CalendarEvent[] | CompanyCalendarResponse>
...
export async function fetchResignations(employeeId?: string): Promise<Resignation[]> {
  const response = await apiClient.get<ApiSuccessResponse<Resignation[]>>(
    `${LEAVE_EXIT_PREFIX}/resignations`,
    {
...
export async function submitResignation(payload: SubmitResignationPayload): Promise<Resignation> {
  const response = await apiClient.post<ApiSuccessResponse<Resignation>>(
    `${LEAVE_EXIT_PREFIX}/resignations`,
    payload,
  );
  return unwrap(response);
...
export async function withdrawResignation(id: string): Promise<Resignation> {
  const response = await apiClient.post<ApiSuccessResponse<Resignation>>(
    `${LEAVE_EXIT_PREFIX}/resignations/${id}/withdraw`,
  );
  return unwrap(response);
}

export async function fetchExitProcess(id: string): Promise<ExitProcess> {
  const response = await apiClient.get<ApiSuccessResponse<ExitProcess>>(
    `${LEAVE_EXIT_PREFIX}/exit/processes/${id}`,
  );
  return unwrap(response);
}

export async function completeExitItem(id: string, notes?: string): Promise<ExitChecklistItem> {
  const response = await apiClient.post<ApiSuccessResponse<ExitChecklistItem>>(
    `${LEAVE_EXIT_PREFIX}/exit/checklist/${id}/complete`,
    { notes },
  );
  return unwrap(response);
}
