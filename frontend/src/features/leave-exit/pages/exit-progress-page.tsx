...
export function ExitProgressPage() {
  ...
  return (
    <div className="space-y-6">
      <LeaveExitPageHeader title="Exit Progress" description="Track HR, IT, finance, and admin clearance checklist." />
      <LeaveExitNav />
      ...
      {data ? (
              ...
              .map((item) => (
                <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3">
                  <div>
                    ...
                    <p className="text-xs capitalize text-muted-foreground">{item.category.replace(/_/g, ' ')}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <StatusBadge status={item.status} />
                    {item.status !== 'completed' ? (
                      <Button size="sm" disabled={complete.isPending} onClick={() => void complete.mutateAsync({ id: item.id })}>
                        Mark Complete
                      </Button>
                    ) : null}
                  </div>
        ...
        <p className="text-muted-foreground">Enter an exit process ID to view clearance progress.</p>
      )}
    </div>
  );
}
