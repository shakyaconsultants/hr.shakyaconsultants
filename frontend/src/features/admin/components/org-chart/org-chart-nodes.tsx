...
export function OrgChartNodeCard({ title, subtitle, meta, icon, accent = 'branch', className }: NodeCardProps) {
  const accentStyles = {
    company:
      ...
      <div className="flex items-start gap-3">
        <div
          className={cn(
            ...
            accent === 'company' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground',
          )}
        >
          {icon}
        </div>
...
export function OrgChartCompanyNode({ name, code, employeeCount }: { name: string; code: string; employeeCount: number }) {
  return (
    <OrgChartNodeCard
      accent="company"
      title={name}
...
export function OrgChartBranchNode({ name, code, employeeCount }: { name: string; code: string; employeeCount: number }) {
  return (
    <OrgChartNodeCard
      accent="branch"
      title={name}
